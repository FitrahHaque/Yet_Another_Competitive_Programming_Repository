...
/********************************************* this is boring *********************************************/
vector<int> subtree,vertex;
vector<vector<pair<int,int>>> grp;
int ans = 0;
int sub(int node=1,int par=0){
    int p = 1;
    for(auto v: grp[node]){
        if(v.first==par) continue;
        p += sub(v.first,node);
    }
    return subtree[node] = p;//#of vertices.
}
void DFS(int node=1,int par=0,ll dist=0LL){
    for(auto v:grp[node]){
        if(par == v.first) continue;
        ll sum = max(dist + v.second,(ll)v.second);
        if(sum > vertex[v.first]){
            ans += subtree[v.first];
        }
        else{
            DFS(v.first,node,sum);
        }
    }
}
